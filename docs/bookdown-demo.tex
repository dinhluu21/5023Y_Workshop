...
When typed in-line within a section of what would otherwise be Markdown text, it knows to produce an r output instead 2021-11-26
...
## $ penguin_id        <chr> "N1A1", "N1A2", "N2A1", "N2A2", "N3A1", "N3A2", "...
\end{verbatim}
...
We first met \texttt{NA} back in (\ref{missing-values---na}) and you will hopefully have noticed, either here or in those previous chapters, that missing values \texttt{NA} can really mess up our calculations. There are a few different ways we can deal with missing data:
...
Enjoy this stupid reward from \citet{R-ggcats}
