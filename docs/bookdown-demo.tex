...
When typed in-line within a section of what would otherwise be Markdown text, it knows to produce an r output instead 2022-01-20
...
If you got completely stuck, or had a go but want to check your answers follow below:
...
\textbf{With the information above, how would you present a short sentence describing the average different in height?}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/project} \caption{An example of a typical R project set-up}\label{fig:unnamed-chunk-346}
\end{figure}

It might suprise you to know that there is no prebuilt function for standard error in R
...
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(Sp\_list)) \{                                    }\CommentTok{\# For every item along the length of Sp\_list we want R to perform the following functions}
\NormalTok{  data }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(Sp\_list[i])                               }\CommentTok{\# Create a dataframe for each species}
\NormalTok{  sp.name }\OtherTok{\textless{}{-}} \FunctionTok{unique}\NormalTok{(data}\SpecialCharTok{$}\NormalTok{Common.Name)                             }\CommentTok{\# Create an object that holds the species name, so that we can title each graph}
\NormalTok{  plot }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(data, }\FunctionTok{aes}\NormalTok{ (}\AttributeTok{x =}\NormalTok{ year, }\AttributeTok{y =}\NormalTok{ abundance)) }\SpecialCharTok{+}               \CommentTok{\# Make the plots and add our customised theme}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{2}\NormalTok{, }\AttributeTok{colour =} \StringTok{"\#00868B"}\NormalTok{) }\SpecialCharTok{+}                                                
    \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =}\NormalTok{ lm, }\AttributeTok{colour =} \StringTok{"\#00868B"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"\#00868B"}\NormalTok{) }\SpecialCharTok{+}          
    \FunctionTok{theme\_custom}\NormalTok{() }\SpecialCharTok{+}
...
\NormalTok{  my\_plots[[i]] }\OtherTok{\textless{}{-}}\NormalTok{ plot }\CommentTok{\# makes a list of all the plots generates}
  \FunctionTok{print}\NormalTok{(plot) }\CommentTok{\# prints each plot out as it is made}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{wrap\_plots}\NormalTok{(my\_plots)}\SpecialCharTok{+}\FunctionTok{plot\_layout}\NormalTok{(}\AttributeTok{design=}\NormalTok{layout) }\CommentTok{\#wrap\_plots function from patchwork can take a list of ggplots}
\end{Highlighting}
\end{Shaded}

\textbf{What if you want to write a loop to save all four plots at once - can you modify the script to do this?}
...
\NormalTok{    \}}
  ...
  \FunctionTok{ggsave}\NormalTok{(plot, }\AttributeTok{file=}\FunctionTok{paste}\NormalTok{(}\StringTok{"figure/"}\NormalTok{, sp.name, }\StringTok{".png"}\NormalTok{, }\AttributeTok{sep=}\StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{dpi=}\DecValTok{900}\NormalTok{) }\CommentTok{\# use paste to automatically add filename}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Coding club. \url{https://ourcodingclub.github.io/tutorials/funandloops/\#function}
\end{itemize}

\url{https://epirhandbook.com/en/iteration-loops-and-lists.html}
