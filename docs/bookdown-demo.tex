...
When typed in-line within a section of what would otherwise be Markdown text, it knows to produce an r output instead 2022-02-24
...
The least squares model first quantifies the total amount of variation (total sum of squares, SST) by measuring the differencea from the individual data points to a reference point or `intercept' (see below). The intercept is arbitrary, as we saw in our regression chapter it defaults to when \emph{x} = 0, but can be shifted to anywhere within our data. In an ANOVA analysis it is normally set to the mean of one of the categories.
