...
The least squares model first quantifies the total amount of variation (total sum of squares, SST) by measuring the differencea from the individual data points to a reference point or 'intercept' (see below). The intercept is arbitrary, as we saw in our regression chapter it defaults to when *x* = 0, but can be shifted to anywhere within our data. In an ANOVA analysis it is normally set to the mean of one of the categories.  
...
darwin_reordered <- darwin %>% group_by(type) %>% arrange(`...1`, .by_group = TRUE) %>% ungroup() %>% mutate(order = c(1:30)) 

lsmodel1 <- lm(height ~ type, data = darwin_reordered)
darwin_augmented <- broom::augment(lsmodel1, darwin_reordered)

SST <- darwin_augmented %>% ggplot(aes(x=order, y=height, colour=type))+geom_segment(aes(x=order, xend=order, y=height, yend=20.192), linetype="dashed", colour="black")+geom_point()+scale_colour_manual(values=c("darkorange", "forestgreen"))+theme_classic()+labs(y="Height in inches", x="Data ordered by type", colour="Pollination type")+geom_hline(aes(yintercept = 20.192), colour="blue")+ggtitle("Sum of Squares Total (SST)")

SSR <- darwin_augmented %>% ggplot(aes(x=order, y=height, colour=type))+geom_segment(aes(x=order, xend=order, y=20.192, yend=.fitted), linetype="dashed", colour="black")+geom_point(alpha=0.4)+scale_colour_manual(values=c("darkorange", "forestgreen"))+theme_classic()+labs(y="Height in inches", x="Data ordered by type", colour="Pollination type")+geom_hline(aes(yintercept = 20.192), colour="blue")+geom_segment(aes(x=16, xend=30, y=.fitted, yend=.fitted), colour="blue")+ggtitle("Sum of Squares Regression (SSR)")

SSE <- darwin_augmented %>% ggplot(aes(x=order, y=height, colour=type))+geom_segment(aes(x=order, xend=order, y=height, yend=.fitted), linetype="dashed", colour="black")+geom_point()+scale_colour_manual(values=c("darkorange", "forestgreen"))+theme_classic()+labs(y="Height in inches", x="Data ordered by type", colour="Pollination type")+geom_hline(aes(yintercept = 20.192), colour="blue")+geom_segment(aes(x=16, xend=30, y=.fitted, yend=.fitted), colour="blue") +ggtitle("Sum of Squares Error (SSE)")
...
The data can be retrieved from [GitHub Classrooms](https://classroom.github.com/a/KZvnm46x), you should set up a new project in the shared workspace on RStudio Cloud. 
...
<details><summary>**EXERCISE ** With the information above can you **import and tidy the data**? *Hint: as well as checking the data, you may need to use `pivot_longer` to put the data in tidy format*.</summary>
...
<details><summary>**EXERCISE -** With the data tidied can you produce a **linear model**, check its assumptions and interpret its findings?**</summary>
...
<details><summary>**EXERCISE -** Can you produce a suitable **Figure** for this dataset?</summary>
...
<details><summary>**EXERCISE -** Can you write a summary of the **Results**?</summary>
